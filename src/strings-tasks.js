...
function removeFirstOccurrences(str, value) {
  ...
  if (str.indexOf(value) >= 0) {
    result = str.slice(0, index) + str.slice(index + value.length, str.length);
  } else {
    result = str;
  }
  return result;
...
function isPalindrome(str) {
  ...
  for (let i = 0; i < punctuationMarks.length; i += 1) {
    cleanStr = cleanStr.replaceAll(punctuationMarks[i], '');
  }
  for (let i = 0; i < cleanStr.length; i += 1) {
    if (
      cleanStr[i].toUpperCase() !==
      cleanStr[cleanStr.length - 1 - i].toUpperCase()
    ) {
      return false;
    }
  }
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (copyStr[i] === copyStr[i].toUpperCase()) {
      changedStr += copyStr[i].toLowerCase();
    } else {
      changedStr += copyStr[i].toUpperCase();
    }
  }
  return changedStr;
}
...
function extractNameFromTemplate(value) {
  ...
  return value.slice(firstSpaceIndex + 1, exclamationIndex);
}
...
function unbracketTag(str) {
  ...
  return str.slice(openingBracket + 1, closingBracket);
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    } else if (alphabetCap.includes(str[i])) {
      const index = alphabetCap.indexOf(str[i]);
      if (index < 13) {
        res += alphabetCap[index + 13];
      } else {
        res += alphabetCap[index - 13];
      }
    } else {
